...
const SalesPriceCalculator = () => {
  ...
  const calculatePrice = (): CalculationResult | null => {
    ...
    try {
      ...
      // 선택한 순도의 마진율 가져오기
      const currentMarginPercent = marginSettings[inputs.purity];
    ...
    <div className="space-y-6">
      ...
      {showSettings && (
        <Card className="border-border">
          <CardHeader>
            <CardTitle>금시세 및 마진 설정</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <h3 className="text-lg font-semibold">금시세 설정 (3.75g 기준)</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold">최대 마진 설정 (%)</h3>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4 p-6 bg-muted rounded-lg border border-border">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-semibold">일반 판매가</h3>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">+{formatNumber(result.regularProfit)}원</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">수익률</span>
                      <span className="font-semibold">{result.regularProfitRate}%</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 p-6 bg-muted rounded-lg border border-border">
                <div className="flex items-center gap-2">
                  <TrendingUp className="h-5 w-5" />
                  <h3 className="text-lg font-semibold">최대 할인가</h3>
                </div>
                <div className="space-y-3">
                  <div className="text-3xl font-bold">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">순이익</span>
                      <span className="font-semibold">+{formatNumber(result.discountProfit)}원</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">수익률</span>
                      <span className="font-semibold">{result.discountProfitRate}%</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="text-sm text-muted-foreground mt-1">
                  (금값: {formatNumber((() => {
                    ...
                    return currentGoldPrice * (weight / 3.75) * purityRatios[inputs.purity];
                  })())}원 + 공임: {formatNumber(parseFloat(inputs.laborCost || '0'))}원 + 기본마진: {formatNumber(marginSettings.baseCost)}원)
                </div>
                <div className="text-xs text-muted-foreground mt-2">
                  계산식: {formatNumber(inputs.priceType === 'vat' ? goldPrice.vatPrice : goldPrice.cashPrice)}원 × ({inputs.weight}g ÷ 3.75g) × {inputs.purity === '14k' ? '0.6435' : inputs.purity === '18k' ? '0.825' : '1'} + {formatNumber(parseFloat(inputs.laborCost || '0'))}원 + {formatNumber(marginSettings.baseCost)}원
                </div>
              </div>
            </div>
          </CardContent>
